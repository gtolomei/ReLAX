...
learning_rate_actor_min=1e-6
learning_rate_actor_param_min=1e-6
...
agent = agent_class(
                   ...
                   seed=seed)
agent.load_models("best0.08_step5")
...
for k in range(episodes):
    ...
    if k % 100 == 0:
        ...
        if(count>=5 and k>5000):
            ...
            count=0
        np.savetxt("acc_tr0.08_step5.txt",acc_tr)
        np.savetxt("reward_tr0.08_step5.txt",reward_record_tr)
        if(k%300==0 and k>1666):
            #print("K",k)
            ...
            np.savetxt("acc_val0.08_step5.txt",acc_val)
            ...
            if(r_test>acc_test_best):
                agent_val.save_models("best0.08_step5_tune")
                acc_test_best=r_test
            if(r_test>0.7):
                agent_val.save_models("Tune_model0.08_step5"+str(np.round(r_test,2))+str(np.round(r_val,2)))
            acc_test.append(r_test)
            np.savetxt("acc_test0.08_step5.txt",acc_test)
