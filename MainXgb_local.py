# -*- coding: utf-8 -*-
"""
Created on Fri Jan 14 01:10:31 2022
...
agent = agent_class(
                   observation_dim, action_dim,action_high,action_low,args.rule,
...
index_list=[i for i in range(X1.shape[0])]
#index_list=[i for i in range(6)]
best_set=[]

for index in index_list:
    print("Num",index)
    X_test=np.expand_dims(X1[index,:],axis=0)
    Y_test=np.array([Y1[index]])
    X_val=np.expand_dims(X1[index,:],axis=0)
    Y_val=np.array([Y1[index]])
    #agent.load_models("model0.08_step30.810.79")
    ...
    #episodes=5000
    x_best=np.array([0]*len(X1[index,:]))
    for k in range(episodes):
        ...
        if(np.random.uniform(0,1)<0.5):
            ...
            i=0
            x=np.random.randn(X_tr.shape[1])*0.1+X1[index,:] 
        ...
        for j in range(max_steps):
            ...
            if(terminal and flag==1):
                dist1=np.sum(np.abs(next_state.numpy()-X1[index,:]))
                dist2=np.sum(np.abs(x_best-X1[index,:]))
                if(dist1<dist2):
                    x_best=next_state.numpy()
        ...
        if k % 100 == 0:
            ...
            if(k%300==0 and k>1600):
                #print("K",k)
                r_val,dist_val=validate(agent_val,observation_dim,action_dim,balance_factor,max_steps,model,X_val,Y_val,val_size)
                acc_val.append(r_val)
                np.savetxt("acc_val0.08_step3.txt",acc_val)
                r_test,dist_val=validate(agent_val,observation_dim,action_dim,balance_factor,max_steps,model,X_test,Y_test,val_size)
                acc_test.append(r_test)
                np.savetxt("acc_test0.08_step3.txt",acc_test)
                print('{0:5s} R:{1:.4f} r100:{2:.4f} rtr{3:.2f} rval{4:.2f} rtest{5:.2f} dist{6:.2f}'.format(str(k), total_reward / (k + 1), np.array(returns[-100:]).mean(),np.array(label[-100:]).mean(),r_val,r_test,dist_val))
            elif(k%100==0):
                print('{0:5s} R:{1:.4f} r100:{2:.4f} rtr{3:.2f}'.format(str(k), total_reward / (k + 1), np.array(returns[-100:]).mean(),np.array(label[-100:]).mean()))
            else:
                pass
    best_set.append(x_best)
    best_list=pd.DataFrame(best_set)
    best_list.to_csv("Best_list_obe.csv")   
