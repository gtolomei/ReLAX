...
class RingBuffer(object):
    def __init__(self, maxlen, shape, dtype='float32'):
        ...
        self.start = 0
        self.length = 0
        print("maxlen",maxlen,"shape",shape)
...
class Memory(object):
    ...
    def sample(self, batch_size, random_machine=np.random):
        #print("sample", self.importance.data.shape[0:self.nb_entries])
        #print("nb-entries",self.nb_entries)
        # Draw such that we always have a proceeding element.
        # batch_idxs = random_machine.random_integers(self.nb_entries - 2, size=batch_size)
        #print("weight")
        batch_idxs=np.array(random.choices(population=np.arange(self.nb_entries),weights=np.squeeze(self.importance.data[0:self.nb_entries]),k=batch_size))
        #print("sample")
        #print(batch_idxs)
    ...
    def append(self, error, state, action, reward, next_state, next_action=None, terminal=False, training=True):
        if not training:
            return
