...
class PDQNAgent(Agent):
    ...
    def __init__(self,
        ...
        self.priori_parameter=priority
    ...
    def act(self, state,action_ava):
        with torch.no_grad():
            ...
            action,action_parameters=constraint(action,action_parameters,self.rule)
    ...
    def _add_sample(self, state, action, reward, next_state, next_action, terminal):
        ...
        #actions_combined = torch.from_numpy(action.to(self.device)
        actions = int(action[0])
        #action_parameters = torch.from_numpy(actions_combined[1:])
        action_parameters=torch.from_numpy(action[1:]).float().to(self.device)
        Q_a = self.actor.forward(state_t.unsqueeze(0), action_parameters.unsqueeze(0)).data
        Q_a_target=self.actor_target.forward(state_t.unsqueeze(0), action_parameters.unsqueeze(0)).data
        if terminal:
            t=reward
        else:
            t=reward+self.gamma*torch.max(Q_a_target)
        #print("Q_a",Q_a,"t",t)
        error=abs(Q_a[0][actions]-t).data.cpu().numpy()
        #print("error",error)
        error=(error + 0.01) ** self.priori_parameter
        self.replay_memory.append(error,state, action, reward, next_state, terminal=terminal)
